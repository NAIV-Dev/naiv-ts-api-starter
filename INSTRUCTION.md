...
On this project, you should only working on *.naiv files, all *.ts files inside `implementation` folder, `.env.example`, `.env`, and utility folder. Do not modify any files inside `types` folder or any files that generated by codegen. You can create helper/utility files in new `utility` folder (create if it doesn't exist). Do not remote `.env.example` file.
